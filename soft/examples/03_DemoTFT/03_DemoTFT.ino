...
#define VERSION   0.3  // Номер версии ПО
  ...
  #define TFT_DC        9  // Команды
#endif  
Adafruit_ST7789 display = Adafruit_ST7789(TFT_CS, TFT_DC, TFT_RST);  // Объявление дисплея
...
void setup() {
  Serial.begin(115200);
  Serial.println("ST7789 graphics demo");
// Инициалаизация дисплея
 #if (HARDWARE == 120)
  display.init(240, 240, SPI_MODE3);           // Новая версия
#else  
  display.init(240, 240, SPI_MODE2);           // Init ST7789 240x240 Параметр SPI_MODE2 ОБЯЗАТЕЛЕН! (без него дисплей не рабоатет!) для версии 110
#endif
...
//  display.fillScreen(0); // Clear screen
  ...
  // Пауза со шкалой 
   ...
   {
    display.fillRect(18+2*i,220, 2, 6, ST77XX_RED);
    delay(10);
   }

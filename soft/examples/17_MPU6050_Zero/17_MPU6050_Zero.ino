/*
IMU Zero
Калибровка MPU6050 для компенсации отклонений или ошибок измерений, которые могут возникнуть из-за таких факторов, как смещение гироскопа и акселерометра.
При запуске этого примера код корректирует значения смещения осей ускорения (X, Y, Z) и вращения (гироскоп) датчика MPU6050,
чтобы обеспечить максимально точные показания,
когда устройство находится в состоянии покоя.
Для получения лучших результатов учтите:
1. Модуль MPU6050 работает исправно.
2. Поместите MPU6050 на ровную горизонтальную поверхность и оставьте его работать в течение
5-10 минут, чтобы стабилизировать температуру.
4. Находитесь в месте, где сила тяжести составляет 1g.
Во время выполнения будет сгенерировано около 10 выходных сигналов, показывающих, что для каждого из 6
желаемых смещений:
- Сначала попробуйте найти две оценки, одну слишком низкую и одну слишком высокую.
- Сближение размеров до тех пор, пока размер кронштейна не станет неизбежным.
Линия чуть выше надписи «Готово» (на это потребуется несколько минут) описывает
оптимальные смещения для ускорения по осям X, Y, Z, гироскопа по осям X, гироскопа по осям Y и гироскопа по осям Z соответственно.
*/
